subscribe = Berlangganan
back = Kembali
next = Lanjut
language = Bahasa
get_bitcoin = Dapatkan BTC
my_wallets = Dompet Saya
withdraw = Tarik
tasks = Tugas
invite_friends = Undang teman
change = Ubah
main_menu = Menu Utama
withdrawal = Penarikan ke
mining = Penambangan
darts = Darts
basketball = Bola Basket
search_for_block = Mulai mencari blok
throw_dart = Lempar dart
language_changed = Bahasa berhasil diubah!
enter_new_wallet = Masukkan nomor dompet baru Anda
wallet_changed = Dompet berhasil diubah!
game_cooldown = Anda sudah memainkan permainan ini hari ini. Coba lagi besok!
darts_win = 🎉 Selamat! Anda berhasil mengenai target dan memenangkan 0.0001 BTC! Kerja bagus!
darts_lose = 😕 Sayangnya, Anda gagal hari ini. Jangan menyerah, coba lagi besok!
start_mining = Proses penambangan dimulai
mining_success_end = 🎉 Blok ditemukan! Anda telah mendapatkan 0.0001 BTC.
insufficient_funds = 🔴 Saldo Anda tidak mencukupi.
main_menu_caption = <b>CRYPTO QUEST -> Menu Utama</b>

    Anda: <b>#SWK58XOH</b>
    Saldo: <b>{$balance}BTC</b>
    Ditarik: <b>0BTC</b>
    Peringkat teratas: <b>Tidak ada</b>

    <a href="https://t.me/+h9EN6YjY--05YjEy">💬 <b>Obrolan</b></a>
mining_game_caption = <b>CRYPTO QUEST -> Dapatkan BTC</b>

    <b>Deskripsi singkat proses penambangan:</b>
    <code>Ini adalah proses penyelesaian masalah matematika kompleks menggunakan algoritma kriptografi SHA-256. Para penambang memverifikasi transaksi dan membuat blok baru untuk blockchain dengan menemukan hash yang unik.</code>

    <b>Mengapa Anda bisa mendapatkan bitcoin dari ini:</b>
    <code>Para penambang mendapatkan bitcoin karena berhasil menambahkan blok ke blockchain. Ini mendorong peserta jaringan untuk menjaga operasinya dan keamanannya.</code>
darts_game_caption = <b>CRYPTO QUEST -> Dapatkan BTC</b>

    <b>🎯 Selamat datang di permainan Darts!</b>
    Uji ketepatan Anda dan dapatkan <b>BTC</b>!
    Mengenai target dan dapatkan <b>0.0001BTC</b> untuk setiap lemparan yang berhasil.

    <i>Permainan ini tersedia sekali sehari, jadi fokuslah dan ambil hadiah Anda!</i>
    Siap untuk melempar dart dan menang? Coba sekarang!

gamezone_caption = <b>CRYPTO QUEST -> Dapatkan BTC</b>

    <b>Selamat datang di zona permainan! 🎮</b>

    Pilih salah satu mini-game seru dan dapatkan <b>BTC</b> sekarang:
    ⛏️ <b>Penambangan</b> – pergi ke tambang virtual dan dapatkan bitcoin!
    🎯 <b>Darts</b> – ketepatan sangat penting! Mengenai target dan dapatkan hadiah Anda.
    🏀 <b>Bola Basket</b> – lempar bola ke keranjang dan menangkan bitcoin.

    <i>Setiap permainan adalah kesempatan untuk meningkatkan modal kripto Anda!</i>

    <b>Anda dapat memainkan setiap permainan sekali sehari!</b>

    Pilih permainan favorit Anda dan mulailah menghasilkan <i><b>BTC</b></i> sekarang!

user_wallets_caption = <b>CRYPTO QUEST -> Dompet</b>

    Masukkan nomor dompet Anda untuk menarik dana. Harap periksa keakuratan data yang dimasukkan untuk menghindari kesalahan dalam transaksi.

    <b>BITCOIN:</b> {$bitcoin_wallet}
    <b>TON:</b> {$ton_wallet}
    <b>TRC20:</b> {$trc20_wallet}

withdraw_caption = <b>CRYPTO QUEST -> Penarikan</b>

    <b>💼 Penarikan Dana</b>

    Di sini Anda dapat menarik BTC yang telah Anda dapatkan ke dompet yang Anda tentukan. Pastikan saldo Anda cukup untuk melakukan penarikan, dan periksa keakuratan data yang dimasukkan.

    Jumlah minimum untuk penarikan: <b>0.000018BTC (50$)</b>.

    <i>Penarikan dana mungkin memerlukan waktu tergantung pada beban jaringan.</i>

language_caption = <b>CRYPTO QUEST -> Pemilihan Bahasa</b>

    Pilih bahasa untuk memudahkan penggunaan bot.

    Pilihan Anda akan diterapkan ke semua bagian dan menu agar interaksi dengan layanan lebih mudah.

withdraw_request_created = 🟡 <b>Permintaan penarikan berhasil dibuat!</b>

    Sekarang, <b>masukkan jumlah</b> yang ingin Anda tarik dan permintaan Anda akan selesai. Setelah itu, BTC Anda akan dikirim ke dompet yang ditentukan secepat mungkin.

    Nomor dompet Anda:
    <code>{$wallet}</code>

withdraw_request_sent = Permintaan penarikan berhasil dikirim!
